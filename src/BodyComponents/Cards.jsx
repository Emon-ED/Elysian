...
const Cards = ({card}) => {
    const {estate_title,id,property_name,description,price,status,area,location,img,facilities}=card;
   console.log(card)
    return (
        ...
        <div className="card-body items-left text-left">
          ...
          <div className="card-actions flex justify-between items-center">
           <Link to={`/details/${id}`}> <button className="btn btn-primary">Details</button></Link>
            <h2 className="mb-2 font-semibold">Price:<span className="text-green-600"> {price}</span></h2>
          </div>
        </div>
      </div>
    );
